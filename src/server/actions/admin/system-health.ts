 ...
 */
async function fetchDynamicThreshold(
  key: string,
  defaultValue: number
): Promise<number> {
  try {
 ...
 * Reads from /proc/net/dev on Linux (Docker/Azure)
 */
function calculateNetworkDelta(): { rxBytes: number; txBytes: number } {
  try {
    ...
    if (platform === "linux") {
      // Linux/Docker: Read from /proc/net/dev
      const fs = require("fs");
    ...
    // macOS/Windows: Return 0 (not critical for production on Azure)
 ...
 */
async function getNetworkRate(): Promise<{
  percentage: number;
  rateMbps: number;
} | null> {
  try {
    ...
    const rateMbps =
      timeDelta > 0 ? ((totalBytes / timeDelta) * 8) / 1_000_000 : 0;
    ...
    // Fetch dynamic limit (default: 1000 Mbps for D4s_v3)
    const limitMbps = await fetchDynamicThreshold(
      "NETWORK_SPEED_LIMIT_MBPS",
      1000 // D4s_v3: ~1 Gbps
    );
    ...
    const percentage =
      limitMbps > 0
        ? Math.min(Math.round((rateMbps / limitMbps) * 100), 100)
        : 0;
...
export async function getSystemMetrics(): Promise<SystemMetrics> {
  try {
    ...
    try {
      ...
      // Fetch dynamic RAM limit from database (default: 16GB for D4s_v3)
      const ramLimitMB = await fetchDynamicThreshold(
        "RAM_LIMIT_MB",
        16384 // D4s_v3: 16 GB = 16384 MB
      );
