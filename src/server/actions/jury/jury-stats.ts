...
export async function getJuryDashboardStats() {
  ...
  try {
    ...
    // Get stats
    const [totalTeams, pendingSubmissions, retryRequests, gradedToday] =
      await Promise.all([
...
export async function getAllGradedSubmissions() {
  ...
  try {
    ...
    // Fetch submissions that were actually graded by a jury (not just auto-approved)
    // judgedById !== null means a jury member graded this submission
    const submissions = await prisma.submission.findMany({
      where: {
        judgedById: {
          not: null, // Only show submissions graded by a jury
        },
        problem: {
          contestId: { in: assignedContestIds },
        },
