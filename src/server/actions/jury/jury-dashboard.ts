...
export async function getAssignedContests(): Promise<AssignedContest[]> {
  ...
  try {
    ...
    // Get problem-to-contest mapping in single query
    const problemContestMap = await prisma.problem.findMany({
      where: { contestId: { in: assignedContestIds } },
      select: { id: true, contestId: true },
    });

    // Build contest -> pending count map
    const contestPendingMap = new Map<string, number>();
    for (const pc of pendingCounts) {
      const problem = problemContestMap.find((p) => p.id === pc.problemId);
      if (problem) {
        const current = contestPendingMap.get(problem.contestId) || 0;
        contestPendingMap.set(problem.contestId, current + pc._count);
      }
    }

    // Combine with contest data
    const contestsWithCounts = assignments.map((assignment) => ({
      ...assignment.contest,
      pendingCount: contestPendingMap.get(assignment.contest.id) || 0,
    }));
