...
export async function requestRetry(submissionId: string, reason: string) {
  try {
    ...
    // 4. VALIDATION: For auto-approve model, can request retry if:
    //    - Submission was rejected by jury, OR
    //    - Submission was graded by jury with score 0 (unfavorable grading)
    // Cannot request retry if jury hasn't graded yet (judgedById is null)
    const wasGradedByJury = submission.judgedById !== null;
    const wasRejected = submission.status === SubmissionStatus.REJECTED;
    const wasGradedUnfavorably =
      wasGradedByJury && submission.manualScore === 0;

    if (!wasGradedByJury) {
      return {
        success: false,
        error:
          "Cannot request retry: Submission has not been reviewed by jury yet",
      };
    }

    if (!wasRejected && !wasGradedUnfavorably) {
      return {
        success: false,
        error:
          "Can only request retry for rejected or unfavorably scored submissions",
      };
    }
    ...
    const teamName =
      submission.user.team_profile?.display_name || "Unknown Team";
